...
### Fixed
- CI installs the tree-sitter CLI version `package-lock.json` locks instead of the
  latest release, and the lock moves to 0.27.0 with both parsers regenerated by it.
  0.27.0 generates `parser.c` differently from 0.26.11, so with CI on "latest" and
  the lock on 0.26.11 every PR failed the generated-parser check while
  `npm run generate` locally reproduced the committed files exactly.
